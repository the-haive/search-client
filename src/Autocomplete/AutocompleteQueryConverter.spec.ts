...
describe('QueryConverters', () => {
    ...
    it('Should match expectations for REST V4 with default query', () => {
        ...
        const expectedFindUrl = 'http://localhost:9950/RestService/v4/autocomplete?l=1&s=10';

        expect(qc.getUrl('http://localhost:9950/RestService/v4', 'autocomplete', defaultQuery)).toEqual(expectedFindUrl);
        expect(qc.getUrl('http://localhost:9950/RestService/v4/', 'autocomplete', defaultQuery)).toEqual(expectedFindUrl);
    });

    it('Should match expectations for REST V4 with given query', () => {
      let qc = new AutocompleteQueryConverter();
      const expectedFindUrl = 'http://localhost:9950/RestService/v4/autocomplete?l=1&q=test&s=20';

      expect(qc.getUrl('http://localhost:9950/RestService/v4', 'autocomplete', fixedQuery)).toEqual(expectedFindUrl);
      expect(qc.getUrl('http://localhost:9950/RestService/v4/', 'autocomplete', fixedQuery)).toEqual(expectedFindUrl);
    });
