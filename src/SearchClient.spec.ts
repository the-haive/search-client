...
describe('SearchClient basics', () => {
    ...
    it('Should be able to create SearchClient instance', () => {
        const searchClient = new SearchClient('http://localhost:9950')
    ...
    it('Should not throw, even for invalid urls. Not perfect, but avoids an additional dependency.', () => {
        expect(() => {
            const searchClient = new SearchClient('file://localhost:9950')
            expect(typeof searchClient).toBe('object')
        }).not.toThrow()
...
describe('SearchClient settings', () => {
    ...
    it('Should be possible to just set the baseUrl and have the various services acquire sensible defaults.', () => {
        const pClient = new SearchClient('http://dummy') as any
        ...
        expect(pClient.autocomplete.settings.basePath).toEqual('RestService/v4')
    ...
    it('Should be possible to override the basePath for services', () => {
        const pClient = new SearchClient({
            baseUrl: 'http://dummy',
            basePath: 'test',
        }) as any
...
describe('SearchClient filter interface', () => {
    ...
    it("Should not modify _origSettings' query-filters when toggling filters", () => {
        ...
        expect(clientAny._origSettings.query.filters).not.toBe(
            clientAny.settings.query.filters
        )
    ...
    it('Should have working match*, queryText, searchType, findAndCategorize and date interfaces', () => {
        ...
        const now = new Date()
        const qConverter = new CategorizeQueryConverter() as any
        const params = qConverter.getUrlParams(client.query)
        const df = params.find((key: string) => key.split('=')[0] === 'df')
        const dt = params.find((key: string) => key.split('=')[0] === 'dt')
        const dateFrom = new Date(decodeURIComponent(df.split('=')[1]))
        const dateTo = new Date(decodeURIComponent(dt.split('=')[1]))
    ...
    it('Search instance with empty settings should have expected query interfaces', () => {
        ...
        const client = new SearchClient(settings)
    ...
    it('should be able to get correct full url for lookups', () => {
        ...
        const mockFindRequest = jest.fn((url: string) => {
            urlFindResult = url
            return false
        })

        let urlCatResult: string
        const mockCatRequest = jest.fn((url: string) => {
            urlCatResult = url
            return false
        })

        const mockFindSuccess = jest.fn()
        const mockCatSuccess = jest.fn()
